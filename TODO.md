...
**Plan: `plans/talk-2026-09-23.md`.** "Some Candidate Laws of New Nature" — 10 min, live
deck + per-slide `say`-voiced audio, track generated by Humboldt from `laws/*.yaml` and
supervisor-edited. Covers the 7 tier-1 laws (L-001–L-007); L-008–L-016 excluded as too
speculative to present as candidate laws.

**Freeze-immune** — composition from law records is not retrieval, so Phase A proceeds now.

1. **[H] Phase A (by 08-25):** `brief.md` (needs operator inputs — conference name,
   audience, whether Humboldt is introduced as artificial, aspect ratio), `slides.yaml`,
   `prompts/talk.md`, `agent/talk.py`. Generate track v1 + edit pass.
